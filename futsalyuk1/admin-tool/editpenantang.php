...
$id = $_GET['id'];
$data = mysqli_query($conn, "SELECT * FROM penantang WHERE id = $id");
...
if(isset($_POST['submit'])){
	...
	if($insert){
		echo '<script>alert("Berhasil diupdate"); location.href = "daftarpenantang.php"</script>';
	}else{
...
<ul>
   ...
   <div class="col-md-10" style="padding:0px">
      <ol class="breadcrumb" style="margin:0;border-radius:0;">
          <li class="active">Edit Penantang</li>
      </ol>
   </div>
 
   <div class="col-md-10" style="width: 25cm; padding-left: 5cm;">
				<h3 class="text-center">Edit Penantang</h3>
				<form method="post" enctype="multipart/form-data">
					<div class="form-group">
						<label>Status Apply</label>
						<input type="text" class="form-control" name="status_apply" value="<?php echo $data['status_apply'] ?>">
					</div>
					<div class="form-group">
						<label>No.HP Pemesan</label>
						<input type="text" class="form-control" name="no_pemesan" value="<?php echo $data['no_pemesan'] ?>">
					</div>
